...
* (WINDOWS) Запустите `powershell` в папке с приложением (`/web-app` например).
  * `docker build . ` - эта команда попытается собрать контейнер с вашим приложением. Вывод из терминала: ![img_3.png](img_3.png), мы видим что `Successfully built 7880e25a8c88`, это значит что наш контейнер успешно собран.
  * `docker run 7880e25a8c88` - эта команда запускает наше приложение в контейнере. ![img_4.png](img_4.png)
...
![img_6.png](img_6.png)
  * Проверяем состояние наших контейнеров
    * `docker ps` - команда показывает запущенные контейнеры с приложениями, в вашем случае их будет 2. ![img_8.png](img_8.png)
