...
* Напишите [контроллер](https://www.baeldung.com/exception-handling-for-rest-with-spring), для обработки ошибок возвращаемых с модуля `data_base-app`. Ошибки обрабатывать отдельной страничкой, с помощью `@AdviceController`, на стронице должна быть гиперссылка/кнопка, для возврата на главную страницу.
