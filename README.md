...
* ***Обратите внимание, что запросы между сервисами в `Docker` теперь не будут работать, т.к. оба приложения находятся в разных сетях.***
  * Используйте в ваших ссылках из сервиса в сервис, имя сервиса в `Docker-compose` вместо `localhost`.
    * Например: `http://service2:10010/api/`
* Протестируйте с помощью ваших `Postman` коллекций.
    
### 5.2 Задание
* Отделяем базы данных от наших приложений.
  * Создаем директорию для каждой базы данных и создадим конфигурационные файлы.

  ![img.png](.img/img9.png)

  * Файл `database.env` содержит основные настройки для самой бд, аналогичные настройкам в `application.propierties`
  ```
    POSTGRES_USER=<имя пользователя>
    POSTGRES_PASSWORD=<пароль>
    POSTGRES_DB=<имя схемы базы данных>
  ```
  * Файл `security.sql` - это уже описанный вами скрипт, для "боевой" базы, с которой работает ваше приложение.
  * Создайте папку для файлов контейнера базы данных, в которой ваш контейнер будет хранить данные в период бездействия `JE_140_0/web-db/data`.
* ***Обратите внимание, что ваш скрип может не соответствовать диалекту отдельно поднятой базы. Конкретно в нашем случае отдельно поднятой базой будет `PostgreSQL`.***
* Изменяем `docker-compose.yml`:
```yaml
    database:
        container_name: web_db
        image: postgres:13.2 # официальный образ базы данных
        env_file:
        ...
        ports:
          - "5438:5432"   
```
* Обязательно укажите `depend_on` опцию для управления порядком запуска ваших сервисов.
* Обновите `application.propierties` в ваших сервисах, где ссылка указывает на контейнер с базой данных.
* Перечень команд которые вам будут полезны в процессе разработки
...
## Не обязательный раздел, для тех, кому захотелось испытать себя

### 6 Задание
* Разделите проект на 2 независимых приложения, т.е. 2 моно модульных проекта.
* Перепишите `application.propierties` через переменные окружения, что бы файлы `application.propierties` получали все необходимы данные из `docker-compose.yml`.

### 7 Задание
* Интегрируйте [Swagger](https://www.baeldung.com/swagger-2-documentation-for-spring-rest-api) в оба ваших приложения.

### 8 Задание
* Напишите `Spring` приложение (без `Spring-Boot` модулей), которое будет каждые 5 секунд делать запрос к сервису базы данных к контроллерам `getAll()`. 
  * Напишите `JSP` для этого приложения, что бы можно было отслеживать состояние таблиц в реальном времени.